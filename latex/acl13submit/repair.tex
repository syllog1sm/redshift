...
The other difference between the systems is in feature pruning. After each iteration,
we prune all features that have received fewer than five updates. We have not
found this to have a substantial effect on accuracy.

\subsection{Training strategies}

Section \ref{sec:oracle} summarises a recent innovation in the way transition-based
dependency parsers are trained. Their method allows examples of states that
result from previous transition errors to be introduced into training, which is
...
This is the method labelled \emph{static} in \citep{goldberg:12}.

\textbf{Goldberg and Nivre.} The parser is allowed to train from states
resulting from previous incorrect trainsitions. The function determing correctness
...
errors. Any prediction within this set will not result in the Perceptron's
weights.

This is the method labelled \emph{dynamic+explore} in \citet{goldberg:12}.
