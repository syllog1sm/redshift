...
The transition system is complete in the sense that the 
two opposite pairings are unlikely to be useful for projective parsing.
Pushing the stack after a Left-Arc could introduce crossing dependencies, 
and popping the stack after a Right-Arc restricts each word to one rightward child.
...
we will have to either undo a dependency now regarded as incorrect, or add a
...
effect of this is exactly as if the model had correctly chosen \emph{Shift} at
...
In this example, the parser has Shifted \emph{Jack} instead of Right-Arcing it. After
...
word above it, and select the Right-Arc label that the model scored most highly
when the word was incorrectly Shifted.
