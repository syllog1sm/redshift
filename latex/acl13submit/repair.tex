...
\label{ref:shiftless}
\note{This discussion is neat, though I'm not sure its crucial -- if we end up
lacking space, this is a good candidate for removal.}
...
\subsection{Arc-Eager oracle definition}
...
The $c$ denotes the number of gold-standandard arcs that cannot be
recovered once the transition has been applied.
A useful property of the  Arc-Eager transition system is \emph{arc-reachability}:
If a set of arcs which can be extended to a projective tree can be individually
derived from a given configuration, then a projective tree containing all of
the arcs in the set is also derivable from the same configuration \citep{nivre:04}.
Using this property, we can define the oracle in terms of a unary function over
configurations, instead of a binary function over configuration/transition pairs.

Let $R(\state)$ be the set of arcs \emph{reachable} from the current
configuration $\state$, and $G$ the set of gold standard arcs. We 
can then define $c$ as:

\begin{equation}
c = \left| R(\state)\cap G \right|  - \left| R(T(\state))\cap G \right|
\end{equation}
...
\begin{eqnarray}
    ...
    \langle B_{h \in {0...i}}, B_{c \in {0...i}}\;\; \mbox{if}\;\; h \neq c \label{eqn:nmo3}\\
    \langle S_{h \in {1...n}}, S_{c \in 0...{n-1}} \rangle \;\; \mbox{if}\;\; h=c+1\;\; \mbox{and} H[S_c]=\emptyset \label{eqn:nmo4}
\end{eqnarray}

\ref{eqn:nmo1} and \ref{eqn:nmo3} are unchanged from before.
\ref{eqn:nm4}
accounts for the arcs that the non-monotonic reduce move makes recoverable, and
\ref{eqn:nmo2} is adjusted 
...
hard to manually design a heuristic dictating which arcs to construct just so
that they can be removed later, and which arcs not to construct in the first
place.  A dynamic-oracle allows us to encode both options, and let the
learner chose when to take each course. 
...
\note{What is this supposed to be?}
\begin{figure}
\centering
    \begin{verbatim}
...
\note{I'd emphasize also the several runs with different seeds aspect.}
...
\note{I suggest emphasizing that the Baseline is in fact a reimplementation of
Goldberg and Nivre, and is already substantially better than the baseline of a ``plain''
arc-eager system with a static oracle.}

\subsection{Transition systems}

\textbf{Baseline.} The unmodified Arc-Eager transition system with training as
in Goldberg and Nivre 2012.
...
\textbf{NM L.} Arc-Eager with non-monotonic Left-Arc.
