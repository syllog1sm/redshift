...
We can think of two ways to remove a push move from the Arc-Eager transition system.
The first is to remove the Shift move, and always attach words when they are
pushed to the stack. The second is to remove the Right-Arc move, and never attach
words when they are pushed. There is a general problem with these transition systems
that is easier to see when they are examined individually.

In the `shiftless' configuration, we break approximately 50\% of the rightward
dependencies we make, as every word that is pushed onto the stack but whose head
is actually in the buffer will receive as a temporary head whatever word happens to be
on top of the stack at the time. For instance, if a preposition is on top of the stack,
and the next word is a determiner, we will create a dependency between them.
These dependencies need labels, and it is not obvious which one to assign during training.
We could reserve a special label, \emph{null}, for the non-gold dependencies. But then
we should not want to reduce these \emph{null} typed dependencies away, as they are
marked as incorrect --- so there is little difference between this approach and the original
Arc-Eager system.

Removing the Right-Arc is a little better, but labelling the arc that will be added
when the word is reduced is a problem here, as well. We risk having to make the
labelling decision in a state where we have lost the information that makes the choice easy.
If we decide instead to build a special label model, then that again seems little different
from just making a prediction when the word is on top of the stack, and retaining the
ability to revise it later.

The general problem these approaches share is that they do not do a good job
...
The only advantage a single push system would have is in avoiding \emph{commitment}
to one of these prior predictions. Our non-monotonic four move system has that
property, without making its previous predictions any less specific.
