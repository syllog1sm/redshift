...
the case, what utility is gained from the first move?

Instead of defining how pop moves can correct Shift/Right mistakes, an alternate
strategy would be to eliminate the ambiguity. There are two possibilities:
...
on the results of previous decisions. Most of the transition decisions are very
easy, and when the parser does encounter a difficult decision, it is advantageous
to have the information that determines the previous choices condensed
into a dependency structure. As an example of the problem, if the Shift move is
...
arcs differently, its action is roughly isomorphic to the Shift move.}, making
important features uselessly noisy.

The non-monotonic transition system that we propose does not have this problem.
The model makes the Shift vs. Right decisions as normal, and conditions on them --- but
