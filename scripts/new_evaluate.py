...
def is_punct(token):
    return token.label == 'P' or token.label == 'punct'

def main(gold_loc, test_loc):
    gold = ParsedFile(gold_loc)
    test = ParsedFile(test_loc)
    uas_scorer = Scorer(lambda g, t: is_punct(g) or g.is_edit or g.label == 'discourse', eval_uas, gold, test)
    las_scorer = Scorer(lambda g, t: is_punct(g) or g.is_edit or g.label == 'discourse', eval_las, gold, test)
    print uas_scorer.t
